...
export interface LlmWikiClientLike {
  health: LlmWikiApiClient["health"]
  projects: LlmWikiApiClient["projects"]
  files: LlmWikiApiClient["files"]
  fileContent: LlmWikiApiClient["fileContent"]
  reviews: LlmWikiApiClient["reviews"]
  search: LlmWikiApiClient["search"]
  graph: LlmWikiApiClient["graph"]
  rescan: LlmWikiApiClient["rescan"]
}
...
export function createMcpServer(apiClient: LlmWikiClientLike = client): Server {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) =>
    handleToolCall(request.params.name, request.params.arguments, apiClient)
  )
...
export async function handleToolCall(
  name: string,
  rawArgs: unknown,
  apiClient: LlmWikiClientLike = client,
) {
    ...
    switch (name) {
      case "llm_wiki_status": {
        ...
        const projects = await apiClient
          .projects()
          .catch(() => ({ projects: [], currentProject: null }))
        return textResult(JSON.stringify({ ...health, ...projects }, null, 2))
      }
      case "llm_wiki_projects": {
        await assertMcpEnabled(apiClient)
        return textResult(JSON.stringify(await apiClient.projects(), null, 2))
      }
      case "llm_wiki_files": {
        await assertMcpEnabled(apiClient)
        const response = await apiClient.files(projectId(args), {
      ...
      case "llm_wiki_read_file": {
        ...
        const { path, content } = await apiClient.fileContent(projectId(args), relPath)
        return textResult(`# ${path}\n\n${truncateText(content, MAX_TEXT_BYTES)}`)
      }
      case "llm_wiki_reviews": {
        await assertMcpEnabled(apiClient)
        const reviews = await apiClient.reviews(projectId(args), {
      ...
      case "llm_wiki_search": {
        ...
        const search = await apiClient.search(projectId(args), query, {
      ...
      case "llm_wiki_graph": {
        await assertMcpEnabled(apiClient)
        const graph = await apiClient.graph(projectId(args), {
      ...
      case "llm_wiki_rescan_sources": {
        await assertMcpEnabled(apiClient)
        return textResult(JSON.stringify(await apiClient.rescan(projectId(args)), null, 2))
      }
      default:
...
async function assertMcpEnabled(apiClient: LlmWikiClientLike): Promise<void> {
  const health = await apiClient.health()
  if (health.mcpEnabled === false) {
    throw new McpError(
