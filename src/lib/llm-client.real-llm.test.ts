...
type RejectMode =
  | { kind: "accept-origin"; allowedOrigin: string }
  | { kind: "reject-all" }

/**
 * Start a fake Ollama-compatible server on 127.0.0.1. Behavior is
 * scriptable per test:
 *   - "accept-origin": 200 + SSE stream when the request's Origin
 *     equals the configured allowedOrigin; 403 with a body
  ...
  const url = await new Promise<string>((resolve, reject) => {
    server = createServer((req: IncomingMessage, res: ServerResponse) => {
      ...
      req.on("end", () => {
        ...
        const allow =
          mode.kind === "accept-origin" && origin === mode.allowedOrigin
        if (!allow) {
          res.statusCode = 403
...
describe("streamChat against a fake Ollama server (real TCP)", () => {
  it(
    ...
    async () => {
      const server = await startFakeOllamaServer({
        kind: "accept-origin",
        allowedOrigin: EXPECTED_LOCAL_LLM_ORIGIN,
      })
      try {
        const cfg: LlmConfig = {
          provider: "ollama",
  ...
  it(
    ...
    async () => {
      // Sanity check: the fake CORS check is active. We force a
      // wrong-origin scenario by setting the server to accept only
      // http://something-else and confirming streamChat surfaces 403.
      const server = await startFakeOllamaServer({
        kind: "accept-origin",
        allowedOrigin: "http://impossible-mismatch.test",
      })
      try {
        const cfg: LlmConfig = {
          provider: "ollama",
  ...
  it(
    "custom local OpenAI-compat endpoint (LM Studio / llama.cpp) also sends Origin = http://localhost",
    async () => {
      const server = await startFakeOllamaServer({
        kind: "accept-origin",
        allowedOrigin: EXPECTED_LOCAL_LLM_ORIGIN,
      ...
      try {
        ...
        // to customEndpoint. Because this fake server is local, custom
        // OpenAI-compatible requests get the same localhost Origin used
        // for Ollama's CORS allowlist.
