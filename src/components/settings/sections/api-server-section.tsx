...
export function buildMcpClientConfig(
  ...
  const env = allowUnauthenticated
    ? {}
    : { LLM_WIKI_API_TOKEN: MCP_TOKEN_ENV_PLACEHOLDER }
...
export function hasUnsavedApiConfig(
  persistedApiConfig: ApiConfig,
  draft: SettingsDraft,
): boolean {
  return (
...
export function ApiServerSection({ draft, setDraft }: Props) {
  ...
  const sampleMcpConfig = useMemo(() => {
    ...
    return buildMcpClientConfig(mcpEntryPath, draft.apiAllowUnauthenticated)
  }, [draft.apiAllowUnauthenticated, mcpEntryPath])

  const hasUnsavedApiConfigValue = hasUnsavedApiConfig(persistedApiConfig, draft)
      ...
      <div className="space-y-3 rounded-lg border border-border/60 bg-muted/20 p-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <Button
            ...
            disabled={hasUnsavedApiConfigValue}
            className="gap-1.5"
          >
        ...
        <div className="rounded-md border border-border/50 bg-background/50 p-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <Button
              ...
              disabled={hasUnsavedApiConfigValue || !draft.apiMcpEnabled || !mcpEntryPath}
              className="gap-1.5"
            >
              <Copy className="h-3.5 w-3.5" />
              {copiedField === "mcp"
