...
Phase 6 的 PR G 升级为 **Chat Agent Router alignment + multimodal chat**：普通 Chat 尽量贴近 upstream `v0.5.x`，同时保留本 fork 的 Agent SDK sidecar 高级能力。
...
核验结果显示 upstream `v0.5.1` tag 和 `HEAD` 都指向 `cc4b98fc33be11216973ec128e9281d8c2f06b79`。PR G 开工时必须重新运行这两个命令，记录当时最新 tag/commit；如果 upstream 已变化，以新核验结果更新 PR G plan、PR body 和 reviewer packet。

## Scope

PR G 需要评估并按风险分批 port：
...
- 先做 upstream `v0.5.x` delta assessment，再开 PR G。
...
- PR G plan 和 PR body 记录开工时最新 upstream tag/commit。
