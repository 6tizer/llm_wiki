...
1. Batch planner：扫描 source，生成 job DAG。
2. Prepare worker pool：bounded concurrency + profile assignment。
...
6. Progress / ETA / pause / resume / cancel UI。
...
本 SPEC 随当前 docs PR 通过 PR-level gate；详见 [SPEC-0](./spec-0-roadmap-baseline.md) 的统一 gate 摘要。实现 PR 必须重新审查 fault injection、resume/idempotency、partial map-reduce、commit-layer integration 和 shell-neutral progress API。
